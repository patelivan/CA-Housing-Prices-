...
ca_county_income_data <- map_df(2010:2018, function(x) {
  get_acs(geography = "county", variables = c(median.household.income = "B19013_001"),
          state = "CA", survey = 'acs5', year = x, output = 'wide') %>% 
    mutate(year = x)
}) 

# Inner joining the county income with ca_data
ca_data2 <- ca_data %>% separate(GEOID, into = c('County_GEOID', 'Tract_GEOID'), sep = 5) %>% 
  inner_join(ca_county_income_data, 
             by = c('County_GEOID' = 'GEOID',  'year' = 'year'), 
             suffix = c('_tract','_county')) 
  

st_write(ca_data2, 'ca_data.shp', driver="ESRI Shapefile")
